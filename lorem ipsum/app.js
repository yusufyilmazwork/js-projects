...
const inputEl = document.querySelector("#paragraps");
const textArea = document.querySelector(".result");


const expandInput = () => {
    let value = inputEl.value;
    inputEl.style.width = `${value.length  * 10 +30}px`;
}

inputEl.addEventListener("input", expandInput);
...
btn.addEventListener('click', () => {
  ...
  for (let i = 0 ; i < number; i++ ) {
    ...
    textArea.innerHTML += `<p>${randomText}</p> <br>` ;
